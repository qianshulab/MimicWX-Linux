...
def find_runtime_base(pid: int, binary_path: str | pathlib.Path) -> int:
    ...
    # PIE load bias is the start of the file-offset-zero mapping, not the
    # beginning of the executable mapping.  Recent Linux WeChat packages map
    # a large read-only segment before .text, so using the r-x mapping start
    # shifts the breakpoint by tens of megabytes.
...
def capture_passphrase(pid: int | None = None, timeout: int = 120) -> str:
    ...
    raise CaptureError(
        "未能捕获 passphrase，断点可能未触发。请确保重新登录了微信。"
        + (f"\nGDB output:\n{tail}" if tail else "")
    )
